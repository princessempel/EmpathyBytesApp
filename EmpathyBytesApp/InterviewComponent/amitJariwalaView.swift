...
struct amitJariwalaView: View {
    var body: some View {
        amitJariwalaPlayer()
    }
}

struct amitJariwalaView_Previews: PreviewProvider {
...
struct amitJariwalaPlayer : View {
    @State var data : Data = .init(count: 0)
    @State var title = ""
    @State var player : AVAudioPlayer!
    @State var playing = false
    @State var width : CGFloat = 0
    @State var songs = ["amitJariwalaAudio"]
    @State var current = 0
    @State var finish = false
    @State var del = AVdelegateAmitJariwala()
            ...
            VStack(spacing: 40) { // Information stack
                ...
                VStack(spacing: 10) {
                    Image(uiImage: self.data.count == 0 ? UIImage(named: "distmath")! : UIImage(data: self.data)!)
                        ...
                        .cornerRadius(15)
                        .padding()
                    Text("The Invention Studio community began in 2009 in a mailroom and as since grown to become one of the top Makrerspaces in the country. Dr. Amit Jariwala, the Faculty Advisor for the Invention Studio, explains the history and creation of the Flowers Invention Studio. He explains the important role of the student peer instructors and the trust maintained between the student leaders and the university.")
...
class AVdelegateAmitJariwala : NSObject,AVAudioPlayerDelegate{
