...
struct joelView: View {
    var body: some View {
        joelPlayer()
    }
}

struct joelView_Previews: PreviewProvider {
    static var previews: some View {
        joelView()
    }
}

struct joelPlayer : View {
    @State var data : Data = .init(count: 0)
    @State var title = ""
    @State var player : AVAudioPlayer!
    @State var playing = false
    @State var width : CGFloat = 0
    @State var songs = ["joelAudio"]
    @State var current = 0
    @State var finish = false
    @State var del = AVdelegateJoel()
...
class AVdelegateJoel : NSObject,AVAudioPlayerDelegate{
